...
(define jlp "Journal of Logic Programming")
...
(define baader-snyder
  (make-bib
   #:author (authors "Franz Baader" "Wayne Snyder")
   #:title "Unification Theory"
   #:location (journal-location "Handbook of Automated Reasoning"
                                #:volume 1
                                #:pages '(445 532))
   #:date "2001"))
...
(define klein-masters
  (make-bib
   #:author "Casey Klein"
   #:title "Experience with Randomized Testing in Programming Language Metatheory"
   #:location (dissertation-location #:institution "Northwestern University"
                                     #:degree "MS")
   #:date 2009))
...
(define clp-semantics
  (make-bib
   #:author (authors "Joxan Jaffar" "Michael J. Maher"
                     "Kim Marriott" "Peter J. Stuckey")
   #:title "The Semantics of Constraint Logic Programming"
   #:location (journal-location jlp
                                #:volume 37
                                #:number "1-3"
                                #:pages '(1 46))
   #:date 1998))
...
(define equational-problems
  (make-bib
   #:author (authors "Hubert Comon" "Pierre Lescanne")
   #:title "Equational Problems and Disunification"
   #:location (journal-location josc
                               #:volume 7
                               #:pages '(371 425))
   #:date 1989))

(define colmerauer-inequations
  (make-bib
   #:author "Alain Colmerauer"
   #:title "Equations and Inequations on Finite and Infinite Trees"
   #:location (proceedings-location fgcs
                                    #:pages '(85 99))
   #:date 1984))

(define sem-sem
  (make-bib
   #:author (authors "Casey Klein" "Jay McCarthy" "Steven Jaconette"
                     "Robert Bruce Findler")
   #:title "A Semantics for Context-Sensitive Reduction Semantics"
   #:location (proceedings-location aplas
                                    #:pages '(369 383))
   #:date 2012))

(define tata
  (make-bib
   #:author (authors "H. Comon" "M. Dauchet" "R. Gilleron"
                     "C. Loding" "F. Jacquemard" "D. Lugiez"
                     "S. Tison" "M. Tommasi")
   #:title "Tree Automata Techniques and Applications"
   #:url "http://www.grappa.univ-lille3.fr/tata"
   #:date 2007))

(define unification-comparison
  (make-bib
   #:author (authors "Krystof Hoder" "Andrei Voronkov")
   #:title "Comparing Unification Algorithms in First-Order Theorem Proving"
   #:date 2009
   #:location (proceedings-location "KI 2009: Advances in Artificial Intelligence"
                                    #:pages '(435 443))))
...
(define palka-workshop
  (make-bib
   #:author (authors "Michal H. Pa\u0142ka" "Koen Claessen"
...
(define nominal-logic
  (make-bib
   ...
   #:date 2003
   #:location (journal-location iac
                                #:volume 186
                                #:pages '(165 193))))

(define pattern-unification
  (make-bib
   #:author "Temur Kutsia"
   #:title "Unification with Sequence Symbols and Flexible Arity Symbols and Its Extension with Pattern-Terms"
   #:date 2002
...
(define csmith
  (make-bib
   #:author (authors "Xuejin Yang" "Yang Chen" "Eric Eide" "John Regehr")
...
(define compiler-testing
  (make-bib
   #:author (authors "Abdulazeez S. Bourjarwah" "Kassem Saleh")
   #:title "Compiler test case generation methods: a survey and assessment"
   #:date 1997
   #:location (journal-location "Information & Software Technology"
                                #:volume 39
                                #:number 9
                                #:pages '(617 625))))
...
(define feat
  (make-bib
   #:author (authors "Jonas Duregard" "Patrik Jansson" "Meng Wang")
